...
/**
 * 인스타그램 퍼블리셔(INSTAGRAM) - Graph API Content Publishing 2단계.
 *  1) POST /{ig-user-id}/media       (image_url, caption) → creation_id
 *  2) POST /{ig-user-id}/media_publish (creation_id)       → media id
 *
 * image_url 은 우리 공개 엔드포인트 `${baseUrl}/cardnews/{id}.png` - 인스타 서버가 이 URL 을 fetch 한다
...
class InstagramPublisher(
    ...
    override fun publish(post: SocialPost): PublishResult {
        ...
        val imageUrl = "$baseUrl/cardnews/${post.id}.png"
        val caption = buildCaption(post)

        // 1) 미디어 컨테이너 생성
        val creationId = postForm(
            "${ig.graphApiUrl}/${ig.businessAccountId}/media",
            mapOf("image_url" to imageUrl, "caption" to caption, "access_token" to ig.accessToken),
        ...
        log.info("[social] 인스타 게시 완료 postId={} mediaId={}", post.id, mediaId)
